...
// Cek apakah permintaan embedded untuk VS Code Editor iframe
if (!empty($_GET['embedded'])) {
    $openDbId = intval($_GET['open_designer'] ?? 0);
    $embedDb = null;
    foreach ($databases as $d) {
        if ((int)$d['id'] === $openDbId) {
            $embedDb = $d;
            break;
        }
    }
    if (!$embedDb && $openDbId > 0) {
        $sqlEmbed = "SELECT d.*, p.name AS project_name FROM db_list d LEFT JOIN projects p ON p.id = d.project_id WHERE d.id = ?" . ($isAdmin ? "" : " AND d.user_id = ?");
        $stmtEmbed = $conn->prepare($sqlEmbed);
        if ($isAdmin) {
            $stmtEmbed->bind_param("i", $openDbId);
        } else {
            $stmtEmbed->bind_param("ii", $openDbId, $user_id);
        }
        $stmtEmbed->execute();
        $embedDb = $stmtEmbed->get_result()->fetch_assoc();
    }
    if (!$embedDb) {
        http_response_code(404);
        echo "<div style='color:#ef4444; padding:20px; font-family:sans-serif; background:#0b0f19'>Database tidak ditemukan atau Anda tidak memiliki izin akses.</div>";
        exit;
    }
    ?>
<!DOCTYPE html>
...
<title>Table Designer &middot; <?php echo htmlspecialchars($embedDb['db_name']); ?></title>
...
<body>
<?php include __DIR__ . '/partials/db-designer-modal.php'; ?>
<script src="assets/db-designer.js?v=<?php echo @filemtime(__DIR__ . '/assets/db-designer.js'); ?>"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    if (window.DB_DESIGNER) {
        window.DB_DESIGNER.open(<?php echo (int)$openDbId; ?>, '<?php echo htmlspecialchars($embedDb['db_name'], ENT_QUOTES); ?>');
    }
});
</script>
</body>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const openDesignerId = urlParams.get('open_designer');
    if (openDesignerId && window.DB_DESIGNER) {
        const id = parseInt(openDesignerId, 10);
        const btn = document.querySelector(`button[onclick*="openDatabaseDesigner(${id},"]`);
        let dbName = '';
        if (btn) {
            const m = btn.getAttribute('onclick').match(/openDatabaseDesigner\\(\\s*\\d+\\s*,\\s*['"]([^'"]+)['"]/);
            if (m) dbName = m[1];
        }
        window.DB_DESIGNER.open(id, dbName);
    }
});
</script>
